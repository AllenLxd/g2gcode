...
class ProductorderController extends BaseController
{
	...
	public function actionCreate()
	{
		...
		if($data['lamp_bead_code'])
		{
			$arr = explode('-', $data['lamp_bead_code']);
			$data['lamp_bead_code'] = $arr[0];
		}
		$productModel = ProductModel::find()->where(['product_name'=>$data['product_model']])->one();
		if($productModel->ic)
		{
			$icNum = ProductOrder::find()->where(['not in','status', ['complete','close']])->andWhere(['ic'=>$productModel->ic])->sum('ic_number');
			if(!$icNum) $icNum = 0;
			...
			if(!$ic)
			{
				...
				if($ic - $icNum < $data['number']*$productModel->ic_number*$productModel->ic_wastage_rate)
				{
					$message = 'IC可用库存不足！';
					$number = $ic - $icNum;
				}
		...
		$lampNum = ProductOrder::find()->where(['not in','status', ['complete','close']])->andWhere(['order_no'=>$data['lamp_bead_code']])->sum('ic_number');
		if(!$lampNum) $lampNum = 0;
		if(!$lampbead) {
			$message = '没有找到关联的灯珠，请检查是否录入有灯珠库存！';
			$number = 0;
		}
		else 
		{
			if($lampbead->remaining - $lampNum < ($productModel->lamp_number*$data['number']*$productModel->wastage_rate))
			{
				$message = '灯珠可用库存不足！';
		...
		$model->ic_number = $productModel->ic_number * $data['number'] * $productModel->ic_wastage_rate;
		$model->lamp_number = $productModel->lamp_number * $data['number'] * $productModel->wastage_rate;
		$model->lamp_wastage_rate = $productModel->wastage_rate;
		$model->ic_wastage_rate = $productModel->ic_wastage_rate;
		$model->save();
		...
		elseif (isset(Yii::$app->request->post()['repcheck']))
		{
			if(Yii::$app->user->identity->type == 'g2g') 
			{
				...
				if(Yii::$app->request->post()['status'] == 'complete')
				{
					...
					$repertoryModel->username = Yii::$app->user->identity->username;
					$repertoryModel->product_order_id = $model->id;
					$repertoryModel->number = $model->number;
					$repertoryModel->remaining = $model->number;
